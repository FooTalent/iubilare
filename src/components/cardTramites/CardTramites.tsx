  ...
  return (
    <div className={`flex flex-col my-2 rounded justify-between items-center shadow-card-shadow transition-all duration-300 ease-in-out ${isExpanded ? 'bg-747DD6 text-white' : ''}`}>
      <div className="flex flex-row justify align-middle py-5 w-full md:w-[687px] md:h-[146px] xl:w-full xl:px-[53px]">
        ...
        <div className="flex w-1/3 items-center xl:justify-end">
      ...
      <div className={`flex flex-col justify-start transition-all delay-100 duration-700 ease-in-out ${isExpanded ? 'h-auto ' : 'h-0 overflow-hidden'}`}>
        ...
        {subtitle && <p className="mx-5 my-5 text-left font-semibold text-base xl:text-lg text-white">{subtitle}</p>}
        <ul>
          {items.map((item, index) => (
            <li key={index} className="mx-5 my-5 text-left text-base xl:text-lg text-white">{item}</li>
          ))}
        </ul>
      </div>
    </div>
